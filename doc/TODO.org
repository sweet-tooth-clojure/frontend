...
** TODO input, disable while submitting
update input component so that it can have disabled: true when form
state is submitting 
** TODO rename :data-path in stff/initialize-form-from-path
:data-path is confusing because it doesn't tell you what the path is
relative to

some reference to the re-frame db might be better
** TODO sync state tracking and outgoing requests
the overall goal is to make it easy to determine the state of a
relevant sync request

examples:
- loading a route
- deleting an item - progress indicator for deletion
- submitting a form

routing as a kind of serialization: take an object, transform it into
a string

routing as identifier - URL is an address after all

sync state as a way of asking, what is the sync state of this
resource?

store sync state as path? - can't because sync is unaware of path

*** route loading?
unlikely - routes can trigger other syncs, but there's no way to look
up the relationship between the two
*** but 
** TODO better route handling
*** check out reitit
*** revisit the entire system: sync, sync adapter, ajax, etc

** TODO form checking for routes

** TODO route lifecycle hierarchy
*** register changes to fire on ANY change
** TODO add datomic id coercion helpers
** TODO consider adding a window click handler
** TODO move utils in from gp3
** TODO demo a list
*** edit multiple in place
*** delete
** TODO route-specific forms?
*** auto clean up on route change?
** TODO make it easy to iterate on form submission
for example, if I want to handle the login form in a special way, make
it easy to handle that without having to log back out and start the
...
** TODO write shadow-friendly stop function
